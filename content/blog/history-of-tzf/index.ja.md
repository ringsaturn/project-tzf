...
date: '2023-01-31'
description: tzfの進化の過程を紹介。最初のGo実装から始まり、後にPython拡張、現在のRust実装、そしてPyO3によるラッパーに至るまでの流れを説明する。
math: true
post_pdf_url: https://blog-assets.ringsaturn.me/pdf/2023-01-31-history-of-tzf.ja.pdf
summary: tzfの進化の過程を紹介。最初のGo実装から始まり、後にPython拡張、現在のRust実装、そしてPyO3によるラッパーに至るまでの流れを説明する。
tags:
- Python
- Go
- Geo
...
ここまでで実用レベルのタイムゾーンライブラリが完成し、クエリ性能は timezonefinder をわずかに上回りました。 しかし高並発の環境下ではさらに高速化が望まれます。Ray casting は $O(n^2)$ の計算量であるため、実行頻度を極力減らすための索引機構が必要でした。

行政区划処理の経験から RTree を検討しましたが、グローバルデータでは効果が薄く、その理由は:

1. タイムゾーン数は数百程度で、行政区划の数千に比べて少ないため、静的言語では全走査でも大きな負荷にならない。
