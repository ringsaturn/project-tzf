...
| 内存为数十 MB，或没有文件系统 | `NewEmbeddedFinder()` | `EmbeddedFinder::new()` | 原地查询 `full.tzb` 的 `+full` 预发布 wheel（实验性） |
| 边界 ~111 m 以内需要精确结果 | `NewFullFinder()` | `DefaultFinder::new_full()`（仅 git 提供的 `full` feature） | tzfpy 自有索引发布的 `+full` 预发布 wheel（实验性） |
...
| lite `.tzb` 原地查询 | `NewEmbeddedFinder()` | 2.4 ms / 2.4 ms | ~4 MB | ~1.2 µs（预索引命中时 p50 为 333 ns） |
...
- **查询：** 对随机世界城市执行一次 `GetTimezoneName`。原地机制的数值约为 1 µs，因为每次回退到点在多边形内判定时都要从压缩文件中解码几何数据。

## 无文件系统或内存很小的场景

`NewEmbeddedFinder()` 原地读取 lite `.tzb`：除文件字节外堆占用约 30 KB（打开时构建的 chunk 块表和预索引缩放级别范围），合计约 4 MB，加载时不解码几何数据，查询过程不分配内存。适用场景为嵌入式目标、2.4 ms 打开耗时占据延迟预算大部分的缩容到零函数计算，以及内存上限较低的进程。
...
`*os.File` 满足 `io.ReaderAt`，`mmap` 封装同样满足。文件在打开时校验一次，之后查询只读取所需的字节。自 tzf 2.1 起，两种原地后端的查询路径都不持有锁：字节切片后端直接在切片上解码，`ReaderAt` 后端通过一组池化的读取视图解码，因此两者的吞吐量都随核数增长。
...
- `NewEmbeddedFinder()`，其 2.4 ms 打开耗时不依赖核数；
...
`tzf-dist` 分发 `lite.tzm`，因为 `NewDefaultFinder()` 需要读取它。仓库不分发 `full.tzm`：该文件为 63.6 MB，而 `full.tzb` 为 15.26 MB。M profile 在使用它的主机上生成。
...
| 分发渠道 | 包含内容 | 体积 |
| --- | --- | --- |
| Go 模块（`tzf-dist`） | `lite.tzb` + `lite.tzm` + `full.tzb` | 压缩后 2.93 + 8.51 + 12.66 MB，合计约 24.1 MB |
| Rust crate（crates.io） | 仅 `lite.tzb` | ~4 MB |
| Rust crate（git，`full` feature） | `full.tzb` | ~15 MB |
| Python wheel（`tzfpy`） | 扩展模块内的 `lite.tzb` | PyPI 上的 2.0.0 为 2.76 MB，2.1.0b2 预发布版为 3.0 MB（v1 为 4.31 MB） |

Go 模块这一组比 v1 的 protobuf 组合（压缩后约 16 MB）大约 8 MB，因为它同时携带 lite 数据集的两种 profile，且自 `v0.0.2026-c-tzb2` 起使用 64 点 chunk。Python wheel 比 v1 小，是因为移除了 protobuf 解码路径。
...
lite 数据集经过 epsilon 为 0.001 度的拓扑感知 Douglas-Peucker 简化，边界位移上限约为 111 m。在 2026-09-14 快照中，lite 查找器对 154,694 个世界城市的查询结果与完整精度基准相比有 1 处不一致（0.0006%），且该结果对应的 UTC 偏移量相同。
